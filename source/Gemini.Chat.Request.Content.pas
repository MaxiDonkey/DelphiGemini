...
type
  ...
  TFunctionCallParams = class(TDataPartParams)
    ...
    /// </summary>
    function Args(const Value: TJSONObject): TFunctionCallParams;
  end;
  ...
  TFunctionResponseParams = class(TDataPartParams)
    ...
    /// </summary>
    function Response(const Value: TJSONObject): TFunctionResponseParams;

    /// <summary>
    /// Optional. Ordered Parts that constitute a function response. Parts may have different IANA MIME types.
    /// </summary>
    function Parts(const Value: TArray<TFunctionResponsePartParams>): TFunctionResponseParams;
  ...
  TPartParams = class(TJSONParam)
    ...
    /// </summary>
    function FunctionCall(const Value: TFunctionCallParams): TPartParams;

    /// <summary>
    /// The result output of a FunctionCall that contains a string representing the FunctionDeclaration.name
    /// and a structured JSON object containing any output from the function is used as context to the model.
    /// </summary>
    function FunctionResponse(const Value: TFunctionResponseParams): TPartParams;
    ...
    /// </summary>
    function ExecutableCode(const Value: TExecutableCodeParams): TPartParams;

    /// <summary>
    /// Result of executing the ExecutableCode.
    /// </summary>
    function CodeExecutionResult(const Value: TCodeExecutionResultParams): TPartParams;
...
begin
  ...
  var FileData :=
    TFileDataParams.Create
      .MimeType(TMediaCodec.GetMimeType(PathOrUrl))
      .FileUri(PathOrUrl);
...
begin
  ...
  var InlineData :=
    TInlineDataParams.Create
      .MimeType(MimeType)
      .Data(TMediaCodec.EncodeBase64(ABytes));
...
begin
  ...
  var InlineData :=
    TInlineDataParams.Create
      .MimeType(MimeType)
      .Data(TMediaCodec.EncodeBase64(Stream));
...
begin
  if TMediaCodec.IsUri(PathOrUrl) then
    begin
      Stream := TMemoryStream.Create;
      try
        if TMediaCodec.TryUrlToStream(PathOrUrl, Stream, MimeType) then
          begin
            Result := TPartParams.Create;
            var InlineData :=
              TInlineDataParams.Create
  ...
  var InlineData :=
    TInlineDataParams.Create
      .MimeType(TMediaCodec.GetMimeType(PathOrUrl))
      .Data(TMediaCodec.EncodeBase64(PathOrUrl));
