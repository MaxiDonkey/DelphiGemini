...
uses
  System.SysUtils, System.Rtti, Gemini.Exceptions;
...
begin
  ...
  if M = nil then
    raise EGeminiExceptionAPI.CreateFmt('RTTI: member "%s" not found on %s', [Name, Instance.ClassName]);
  ...
  if M is TRttiField then
    ...
    begin
      if not TRttiProperty(M).IsWritable then
        raise EGeminiExceptionAPI.CreateFmt('RTTI: property "%s" on %s is read-only', [Name, Instance.ClassName]);
