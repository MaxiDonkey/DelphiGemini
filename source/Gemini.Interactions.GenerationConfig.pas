...
begin
  if TJSONHelper.TryGetObject(Value, JSONObject) then
    Exit(TGenerationConfigIxParams(Add('speech_config', JSONObject)));
...
begin
  if TJSONHelper.TryGetObject(Value, JSONObject) then
    Exit(TGenerationConfigIxParams(Add('tool_choice', JSONObject)));
...
function TGenerationConfigIxParams.TopP(
  const Value: Double): TGenerationConfigIxParams;
begin
  Result := TGenerationConfigIxParams(Add('top_p', Value));
end;
