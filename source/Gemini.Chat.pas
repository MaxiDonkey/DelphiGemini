...
function TChatRoute.AsyncAwaitCreate(const ModelName: string;
  const ParamProc: TProc<TChatParams>;
...
begin
  Result := TAsyncAwaitHelper.WrapAsyncAwait<TChat>(
    ...
    begin
      AsynCreate(ModelName, ParamProc, CallbackParams);
    end,
    Callbacks);
end;

function TChatRoute.AsyncAwaitCreateStream(const ModelName: string;
  const ParamProc: TProc<TChatParams>;
  const Callbacks: TFunc<TPromiseChatStream>): TPromise<string>;
begin
