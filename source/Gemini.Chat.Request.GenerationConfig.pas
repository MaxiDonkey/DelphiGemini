...
type
  ...
  TThinkingConfig = class(TJSONParam)
    ...
    /// </summary>
    function IncludeThoughts(const Value: Boolean = True): TThinkingConfig;
...
begin
  Result := ResponseSchema(TJSONHelper.StringToJson(Value));
end;
...
begin
  Result := TMultiSpeakerVoiceConfig(Add('speakerVoiceConfigs',
    TJSONHelper.ToJsonArray<TSpeakerVoiceConfig>(Value)));
end;
