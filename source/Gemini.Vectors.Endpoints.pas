...
type
  ...
  public
    ...
    class function RequireDocumentName(const Value, ErrMsg: string): string; overload; static;
    class function RequireDocumentName(const Value: string): string; overload; static;
    class function RequireOperationName(const Value, ErrMsg: string): string; overload; static;
    class function RequireOperationName(const Value: string): string; overload; static;
  end;
...
class function TVectorEndpoints.RequireDocumentName(
  const Value: string): string;
begin
  Result := RequireDocumentName(Value, INVALID_DOCUMENT_NAME);
end;

class function TVectorEndpoints.RequireOperationName(
  const Value: string): string;
begin
  Result := RequireOperationName(Value, INVALID_OPERATION_NAME);
end;

class function TVectorEndpoints.RequireOperationName(const Value,
  ErrMsg: string): string;
begin
  if not IsOperationName(Value) then
    raise EGeminiEndpointError.Create(ErrMsg);
  Result := Value;
end;
