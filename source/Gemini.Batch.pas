...
type
  ...
  public
    ...
    /// <summary>
    /// Downloads a batch response file as JSONL (newline-delimited JSON) and returns the content wrapper.
    /// </summary>
    /// <param name="FileName">
    /// The file resource name (or identifier) to download. This is typically the value returned by the batch
    /// operation response (for example, <c>TOperation.ResponseFile</c>).
    /// </param>
    /// <returns>
    /// A <c>TJsonlDownload</c> instance containing the downloaded JSONL payload in its <c>JSONResponse</c> fingerprint.
    /// </returns>
    /// <remarks>
    /// This method performs a synchronous (blocking) download request.
    /// <para>
    /// • The download endpoint is served from a dedicated API base (internally switching <c>API.Version</c> to
    /// <c>download/v1beta</c> for the duration of the call). To avoid cross-thread interference, the method
    /// serializes access while the version is temporarily changed.
    /// </para>
    /// <para>
    /// • The returned <c>TJsonlDownload</c> can be persisted to disk using <c>TJsonlDownload.SaveToJsonl</c>.
    /// </para>
    /// <para>
    /// • Expected input example: <c>files/abc-123</c>.
    /// </para>
    /// </remarks>
...
begin
  Result := Value.Trim;
  if not Result.StartsWith('batches/') then
    Result := 'batches/' + Result.TrimLeft(['/']);
end;
